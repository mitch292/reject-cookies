import {
  checkForABConsent,
  checkForACookie,
  checkForAliExpressGDPR,
  checkForAWSCCC,
  checkForBoschConsent,
  checkForBorgerCookie,
  checkForCC,
  checkForCCConsentBanner,
  checkForCmplz,
  checkForConsentManager,
  checkForRelyance,
  checkForConsentPro,
  checkForCookieBot,
  checkForCookieConsent,
  checkForCookieFirst,
  checkForCookieHub,
  checkForCookieNotice,
  checkForCookieScript,
  checkForCookieYes,
  checkForDelve,
  checkForDidomi,
  checkForDrCookie,
  checkForEbayGDPR,
  checkForEUCookieCompliance,
  checkForEzoic,
  checkForFides,
  checkForFreeChoice,
  checkForGeneric,
  checkForGetAdBlockCookie,
  checkForGitBookCookies,
  checkForHubSpotCookie,
  checkForHubspotCookie,
  checkForIubenda,
  checkForKetch,
  checkForMooveGDPR,
  checkForNikeCookie,
  checkForOneTrust,
  checkForOneUptimeCookie,
  checkForOsano,
  checkForPopupBox,
  checkForPornhubCookie,
  checkForQuantcast,
  checkForRaiderCookie,
  checkForSilktide,
  checkForSnapcraftCookie,
  checkForSourcepoint,
  checkForSquarespaceCookie,
  checkForSubstackCookie,
  checkForTermly,
  checkForTranscend,
  checkForTrustArc,
  checkForTrustee,
  checkForUCGDPR,
  checkForUserCentrics,
  checkForWebToffee,
  checkForWixCookieBanner,
  checkForWorkableCookie,
  checkForWPPrivacy,
  checkForXCookieBanner,
  checkForXHCookiesAnnounce,
  checkForXHCookiesModal,
  checkForYahooGUCE,
  checkForCookieBar,
  checkForUniConsent,
  checkForMdCookieAllow,
  checkForTohoCookie,
  checkForSnigelCmp,
  checkForShopifyPrivacy,
  checkForMicrosoftWCP,
  checkForMediaFireCookie,
  checkForAmazonCookie,
  checkForMudBlazorCC,
} from './checks';
import {
  closeOrRejectACookie,
  closeOrRejectCC,
  closeOrRejectCmplz,
  closeOrRejectCookieYes,
  closeOrRejectDelve,
  closeOrRejectDidomi,
  closeOrRejectDrCookie,
  closeOrRejectEbayGDPR,
  closeOrRejectFides,
  closeOrRejectFreeChoice,
  closeOrRejectGeneric,
  closeOrRejectIubenda,
  closeOrRejectNikeCookie,
  closeOrRejectOneTrust,
  closeOrRejectPornhubCookie,
  closeOrRejectQuantcast,
  closeOrRejectSquarespaceCookie,
  closeOrRejectTrustArc,
  closeOrRejectTrustee,
  closeOrRejectWixCookieBanner,
  closeOrRejectXCookieBanner,
  closeOrRejectYahooGUCE,
  closeOrRejectCookieBar,
  closeOrRejectAliExpressGDPR,
  closeOrRejectBorgerCookie,
  closeOrRejectConsentPro,
  closeOrRejectCookieConsent,
  closeOrRejectCookieHub,
  closeOrRejectCookieNotice,
  closeOrRejectEzoic,
  closeOrRejectGitBookCookies,
  closeOrRejectHubSpotCookie,
  closeOrRejectKetch,
  closeOrRejectConsentManager,
  closeOrRejectRelyance,
  closeOrRejectHubspotCookie,
  closeOrRejectMdCookieAllow,
  closeOrRejectTohoCookie,
  closeOrRejectSnigelCmp,
  closeOrRejectMooveGDPR,
  closeOrRejectOneUptimeCookie,
  closeOrRejectOsano,
  closeOrRejectPopupBox,
  closeOrRejectRaiderCookie,
  closeOrRejectSubstackCookie,
  closeOrRejectUniConsent,
  closeOrRejectWebToffee,
  closeOrRejectWorkableCookie,
  closeOrRejectShopifyPrivacy,
  closeOrRejectAmazonCookie,
  closeOrRejectMudBlazorCC,
  closeOrRejectXHCookiesAnnounce,
  closeOrRejectXHCookiesModal,
  closeOrRejectGetAdBlockCookie,
  closeOrRejectSnapcraftCookie,
  closeOrRejectCCConsentBanner,
  closeOrRejectSilktide,
  closeOrRejectCookieFirst,
  closeOrRejectABConsent,
  closeOrRejectBoschConsent,
  closeOrRejectEUCookieCompliance,
  closeOrRejectWPPrivacy,
  closeSourcepoint,
  closeTranscend,
  rejectAWSCCC,
  rejectCookieBot,
  rejectCookieScript,
  rejectOrCloseUCGDPR,
  rejectTermly,
  rejectUserCentrics,
  closeOrRejectMicrosoftWCP,
  closeOrRejectMediaFireCookie,
} from './rejectFlows';
import { CookiePopupCheck } from './types';

export const commonCookiePopupChecks: CookiePopupCheck[] = [
  {
    name: 'onetrust',
    check: checkForOneTrust,
    rejectOrClose: closeOrRejectOneTrust,
    successful: false,
  },
  {
    name: 'transcend',
    check: checkForTranscend,
    rejectOrClose: closeTranscend,
    successful: false,
  },
  {
    name: 'cookieyes',
    check: checkForCookieYes,
    rejectOrClose: closeOrRejectCookieYes,
    successful: false,
  },
  {
    name: 'cookiebot',
    check: checkForCookieBot,
    rejectOrClose: rejectCookieBot,
    successful: false,
  },
  {
    name: 'usercentrics',
    check: checkForUserCentrics,
    rejectOrClose: rejectUserCentrics,
    successful: false,
  },
  {
    name: 'didomi',
    check: checkForDidomi,
    rejectOrClose: closeOrRejectDidomi,
    successful: false,
  },
  {
    name: 'trustarc',
    check: checkForTrustArc,
    rejectOrClose: closeOrRejectTrustArc,
    successful: false,
  },
  {
    name: 'dr',
    check: checkForDrCookie,
    rejectOrClose: closeOrRejectDrCookie,
    successful: false,
  },
  {
    name: 'uc-gdpr',
    check: checkForUCGDPR,
    rejectOrClose: rejectOrCloseUCGDPR,
    successful: false,
  },
  {
    name: 'cc',
    check: checkForCC,
    rejectOrClose: closeOrRejectCC,
    successful: false,
  },
  {
    name: 'generic',
    check: checkForGeneric,
    rejectOrClose: closeOrRejectGeneric,
    successful: false,
  },
  {
    name: 'trustee',
    check: checkForTrustee,
    rejectOrClose: closeOrRejectTrustee,
    successful: false,
  },
  {
    name: 'cmplz',
    check: checkForCmplz,
    rejectOrClose: closeOrRejectCmplz,
    successful: false,
  },
  {
    name: 'acookie',
    check: checkForACookie,
    rejectOrClose: closeOrRejectACookie,
    successful: false,
  },
  {
    name: 'sourcepoint',
    check: checkForSourcepoint,
    rejectOrClose: closeSourcepoint,
    successful: false,
  },
  {
    name: 'quantcast',
    check: checkForQuantcast,
    rejectOrClose: closeOrRejectQuantcast,
    successful: false,
  },
  {
    name: 'iubenda',
    check: checkForIubenda,
    rejectOrClose: closeOrRejectIubenda,
    successful: false,
  },
  {
    name: 'fides',
    check: checkForFides,
    rejectOrClose: closeOrRejectFides,
    successful: false,
  },
  {
    name: 'delve',
    check: checkForDelve,
    rejectOrClose: closeOrRejectDelve,
    successful: false,
  },
  {
    name: 'nike-cookie',
    check: checkForNikeCookie,
    rejectOrClose: closeOrRejectNikeCookie,
    successful: false,
  },
  {
    name: 'ebay-gdpr',
    check: checkForEbayGDPR,
    rejectOrClose: closeOrRejectEbayGDPR,
    successful: false,
  },
  {
    name: 'freechoice',
    check: checkForFreeChoice,
    rejectOrClose: closeOrRejectFreeChoice,
    successful: false,
  },
  {
    name: 'wix-cookie-banner',
    check: checkForWixCookieBanner,
    rejectOrClose: closeOrRejectWixCookieBanner,
    successful: false,
  },
  {
    name: 'yahoo-guce',
    check: checkForYahooGUCE,
    rejectOrClose: closeOrRejectYahooGUCE,
    successful: false,
  },
  {
    name: 'x-cookie-banner',
    check: checkForXCookieBanner,
    rejectOrClose: closeOrRejectXCookieBanner,
    successful: false,
  },
  {
    name: 'cookiebar',
    check: checkForCookieBar,
    rejectOrClose: closeOrRejectCookieBar,
    successful: false,
  },
  {
    name: 'uniconsent',
    check: checkForUniConsent,
    rejectOrClose: closeOrRejectUniConsent,
    successful: false,
  },
  {
    name: 'popup-box',
    check: checkForPopupBox,
    rejectOrClose: closeOrRejectPopupBox,
    successful: false,
  },
  {
    name: 'xh-cookies',
    check: checkForXHCookiesModal,
    rejectOrClose: closeOrRejectXHCookiesModal,
    successful: false,
  },
  {
    name: 'cookiescript',
    check: checkForCookieScript,
    rejectOrClose: rejectCookieScript,
    successful: false,
  },
  {
    name: 'awsccc',
    check: checkForAWSCCC,
    rejectOrClose: rejectAWSCCC,
    successful: false,
  },
  {
    name: 'squarespace-cookie',
    check: checkForSquarespaceCookie,
    rejectOrClose: closeOrRejectSquarespaceCookie,
    successful: false,
  },
  {
    name: 'termly',
    check: checkForTermly,
    rejectOrClose: rejectTermly,
    successful: false,
  },
  {
    name: 'pornhub-cookie',
    check: checkForPornhubCookie,
    rejectOrClose: closeOrRejectPornhubCookie,
    successful: false,
  },
  {
    name: 'cookieconsent',
    check: checkForCookieConsent,
    rejectOrClose: closeOrRejectCookieConsent,
    successful: false,
  },
  {
    name: 'workable',
    check: checkForWorkableCookie,
    rejectOrClose: closeOrRejectWorkableCookie,
    successful: false,
  },
  {
    name: 'webtoffee',
    check: checkForWebToffee,
    rejectOrClose: closeOrRejectWebToffee,
    successful: false,
  },
  {
    name: 'borger-cookie',
    check: checkForBorgerCookie,
    rejectOrClose: closeOrRejectBorgerCookie,
    successful: false,
  },
  {
    name: 'ezoic',
    check: checkForEzoic,
    rejectOrClose: closeOrRejectEzoic,
    successful: false,
  },
  {
    name: 'gitbook-cookies',
    check: checkForGitBookCookies,
    rejectOrClose: closeOrRejectGitBookCookies,
    successful: false,
  },
  {
    name: 'oneuptime-cookie',
    check: checkForOneUptimeCookie,
    rejectOrClose: closeOrRejectOneUptimeCookie,
    successful: false,
  },
  {
    name: 'substack-cookie',
    check: checkForSubstackCookie,
    rejectOrClose: closeOrRejectSubstackCookie,
    successful: false,
  },
  {
    name: 'aliexpress-gdpr',
    check: checkForAliExpressGDPR,
    rejectOrClose: closeOrRejectAliExpressGDPR,
    successful: false,
  },
  {
    name: 'cookie-notice',
    check: checkForCookieNotice,
    rejectOrClose: closeOrRejectCookieNotice,
    successful: false,
  },
  {
    name: 'ketch',
    check: checkForKetch,
    rejectOrClose: closeOrRejectKetch,
    successful: false,
  },
  {
    name: 'md-cookie',
    check: checkForMdCookieAllow,
    rejectOrClose: closeOrRejectMdCookieAllow,
    successful: false,
  },
  {
    name: 'toho-cookie',
    check: checkForTohoCookie,
    rejectOrClose: closeOrRejectTohoCookie,
    successful: false,
  },
  {
    name: 'snigel-cmp',
    check: checkForSnigelCmp,
    rejectOrClose: closeOrRejectSnigelCmp,
    successful: false,
  },
  {
    name: 'consentmanager',
    check: checkForConsentManager,
    rejectOrClose: closeOrRejectConsentManager,
    successful: false,
  },
  {
    name: 'relyance',
    check: checkForRelyance,
    rejectOrClose: closeOrRejectRelyance,
    successful: false,
  },
  {
    name: 'osano',
    check: checkForOsano,
    rejectOrClose: closeOrRejectOsano,
    successful: false,
  },
  {
    name: 'raider-cookie',
    check: checkForRaiderCookie,
    rejectOrClose: closeOrRejectRaiderCookie,
    successful: false,
  },
  {
    name: 'moove-gdpr',
    check: checkForMooveGDPR,
    rejectOrClose: closeOrRejectMooveGDPR,
    successful: false,
  },
  {
    name: 'cookiehub',
    check: checkForCookieHub,
    rejectOrClose: closeOrRejectCookieHub,
    successful: false,
  },
  {
    name: 'hubspot-gdpr-alert',
    check: checkForHubspotCookie,
    rejectOrClose: closeOrRejectHubspotCookie,
    successful: false,
  },
  {
    name: 'hubspot-cookie',
    check: checkForHubSpotCookie,
    rejectOrClose: closeOrRejectHubSpotCookie,
    successful: false,
  },
  {
    name: 'consentpro',
    check: checkForConsentPro,
    rejectOrClose: closeOrRejectConsentPro,
    successful: false,
  },
  {
    name: 'xh-cookies-announce',
    check: checkForXHCookiesAnnounce,
    rejectOrClose: closeOrRejectXHCookiesAnnounce,
    successful: false,
  },
  {
    name: 'shopify-privacy',
    check: checkForShopifyPrivacy,
    rejectOrClose: closeOrRejectShopifyPrivacy,
    successful: false,
  },
  {
    name: 'microsoft-wcp',
    check: checkForMicrosoftWCP,
    rejectOrClose: closeOrRejectMicrosoftWCP,
    successful: false,
  },
  {
    name: 'mediafire-cookie',
    check: checkForMediaFireCookie,
    rejectOrClose: closeOrRejectMediaFireCookie,
    successful: false,
  },
  {
    name: 'amazon-cookie',
    check: checkForAmazonCookie,
    rejectOrClose: closeOrRejectAmazonCookie,
    successful: false,
  },
  {
    name: 'mudblazor-cc',
    check: checkForMudBlazorCC,
    rejectOrClose: closeOrRejectMudBlazorCC,
    successful: false,
  },
  {
    name: 'getadblock-cookie',
    check: checkForGetAdBlockCookie,
    rejectOrClose: closeOrRejectGetAdBlockCookie,
    successful: false,
  },
  {
    name: 'snapcraft-cookie',
    check: checkForSnapcraftCookie,
    rejectOrClose: closeOrRejectSnapcraftCookie,
    successful: false,
  },
  {
    name: 'cc-consent-banner',
    check: checkForCCConsentBanner,
    rejectOrClose: closeOrRejectCCConsentBanner,
    successful: false,
  },
  {
    name: 'silktide',
    check: checkForSilktide,
    rejectOrClose: closeOrRejectSilktide,
    successful: false,
  },
  {
    name: 'cookiefirst',
    check: checkForCookieFirst,
    rejectOrClose: closeOrRejectCookieFirst,
    successful: false,
  },
  {
    name: 'eu-cookie-compliance',
    check: checkForEUCookieCompliance,
    rejectOrClose: closeOrRejectEUCookieCompliance,
    successful: false,
  },
  {
    name: 'abconsent',
    check: checkForABConsent,
    rejectOrClose: closeOrRejectABConsent,
    successful: false,
  },
  {
    name: 'bosch-consent',
    check: checkForBoschConsent,
    rejectOrClose: closeOrRejectBoschConsent,
    successful: false,
  },
  {
    name: 'wp-privacy',
    check: checkForWPPrivacy,
    rejectOrClose: closeOrRejectWPPrivacy,
    successful: false,
  },
];
